...
#include "/home/atishays/Desktop/467/ORB_SLAM3/lcmtypes/mbot_ransac_t.hpp"
...
class Handler 
{
    public:
        ...
        void handleRansacMessage(const lcm::ReceiveBuffer* rbuf,
            ...
            {
                std::lock_guard<std::mutex> lk(lcm_mutex);
                ORB_SLAM3::MapDrawer::ransacPoints = msg->inliers;
            }
...
void lcm_ransac_handler(void){
    ...
    Handler handlerObject;
    lcm.subscribe("MBOT_RANSAC_RETURN", &Handler::handleRansacMessage, &handlerObject);
}
...
int main(int argc, char *argv[])
{
    ...
    std::thread lcm_ransac_thread(lcm_ransac_handler);
