...
#include "/home/atishays/Desktop/467/Efficient-RANSAC-for-Point-Cloud-Shape-Detection/lcmtypes/mbot_ransac_t.hpp"
...
class Handler 
{
    public:
        ...
        void handleRansacMessage(const lcm::ReceiveBuffer* rbuf,
            ...
            const mbot_ransac_t *msg)
            {
				update = true;
				std::copy((msg->points).begin(), (msg->points).end(), buffer_vec.begin());
            }
};
...
int main()
{
    ...
    lcm.subscribe("MBOT_RANSAC_SEND", &Handler::handleRansacMessage, &handlerObject);
	
	while(1){
		if(update){
			pc.clear();
			std::copy((buffer_vec).begin(), (buffer_vec).end(), orb_points.begin());

			// for(int i=0;i<10;i++){
			// 	for(int j=0;j<10;j++){
			// 		pc.push_back(Point(Vec3f(i,j,0)));
			// 		pc.push_back(Point(Vec3f(10,i,j)));
			// 		pc.push_back(Point(Vec3f(i,j,20)));
			// 		pc.push_back(Point(Vec3f(i,j,30)));
			// 		pc.push_back(Point(Vec3f(i,j,40)));
			// 	}
			// }

			for(size_t i = 0; i < orb_points.size(); i = i+3){
				pc.push_back(Point(Vec3f(orb_points[i], orb_points[i+1], orb_points[i+2])));
			}
			//std::clock_t start;
			//start = std::clock();
			// set the bbox in pc
			...
			ransacOptions.m_minSupport = 100; // this is the minimal numer of points required for a primitive
			ransacOptions.m_probability = .001f; // this is the "probability" with which a primitive is overlooked

			std::cout << "Set ransacOptions params" << std::endl;
			RansacShapeDetector detector(ransacOptions); // the detector object

			// set which primitives are to be detected by adding the respective constructors
			detector.Add(new PlanePrimitiveShapeConstructor());

			std::cout << "Added Detectors" << std::endl;
			...
			*/
			...
			mbot_ransac_t inlier_msg;
			inlier_msg.num_inliers = pc.size() * 3;

			for(size_t i = remaining; i < pc.size(); ++i){
				inlier_msg.inliers.push_back(pc[i].pos[0]);
				inlier_msg.inliers.push_back(pc[i].pos[1]);
				inlier_msg.inliers.push_back(pc[i].pos[2]);
			}

			lcm.publish("MBOT_RANSAC_RECEIVE", &inlier_msg);
			
			update = false;
			//std::cout << std::clock() - start << " " << CLOCKS_PER_SEC << "\n";
		}
	}
}
