...
if (nPatternFound > 9):
    ...
    # line below was used in previous attempt in fisheye rectification. Had odd reshaping behavior
    #newcameramtx=cv2.fisheye.estimateNewCameraMatrixForUndistortRectify(K,D,(w,h),np.eye(3),balance=0.5)

    mapx,mapy = cv2.fisheye.initUndistortRectifyMap(K,D,np.eye(3),K,(w,h), cv2.CV_16SC2)
