...
std::vector<float> MapDrawer::ransacPoints = std::vector<float>();
...
void MapDrawer::DrawImSeg(){
    glPointSize(mPointSize);
    glBegin(GL_POINTS);
    glColor3f(0.0,1.0,1.0);

    for(size_t i = 0; i < ransacPoints.size(); i += 3){
        glVertex3f(ransacPoints[i], ransacPoints[i+1], ransacPoints[i+2]);
    }
