...
#include "lcmtypes/mbot_rgb_stream_t.hpp"
...
class Handler 
{
    public:
        ...
        void handleRGBStream(const lcm::ReceiveBuffer* rbuf, const std::string& chan, const mbot_rgb_stream_t* msg){
            std::lock_guard<std::mutex> lk(lcm_mutex);
            ...
            cout << "In RGB handler\n";
            imRGB = cv::imdecode(msg->image, cv::IMREAD_GRAYSCALE);
        ...
        void handleDStream(const lcm::ReceiveBuffer* rbuf, const std::string& chan, const mbot_d_stream_t* msg){
            std::lock_guard<std::mutex> lk(lcm_mutex);
            ...
            imD = cv::imdecode(msg->image, cv::IMREAD_GRAYSCALE);
            // std::string frame_name = to_string(msg->timestamp);
            // cv::imwrite("./stream_data/"+frame_name+".jpg", frame);
            //imshow("Frame", frame);
            cout << "Received Image\n";
...
void lcm_rgb_handler(void){
    ...
    if(!lcm.good())
        return;
    Handler handlerObject;
    lcm.subscribe("MBOT_RGB_STREAM", &Handler::handleRGBStream, &handlerObject);
...
void lcm_d_handler(void){
    ...
    if(!lcm.good())
        return;
    Handler handlerObject;
    lcm.subscribe("MBOT_D_STREAM", &Handler::handleDStream, &handlerObject);
...
int main(int argc, char *argv[])
{
    ...
    ORB_SLAM3::System SLAM(
                        argv[1],
                        argv[2],
                        DO_IMU ? ORB_SLAM3::System::IMU_MONOCULAR : ORB_SLAM3::System::RGBD,
                        true);
    ...
    std::thread lcm_imrgb_thread(lcm_rgb_handler);
    std::thread lcm_imd_thread(lcm_d_handler);
    ...
    while(1)
    {
        ...
        condv.wait(lk, []{return found_imrgb == true && found_imd == true;});
